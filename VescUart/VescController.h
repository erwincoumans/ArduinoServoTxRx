...
#define COMM_SET_CUR 6
#define COMM_SET_BRAKE 7
...
{
public:
  VescController(HardwareSerial& theSerial) : controlSerial(theSerial){};
  ~VescController(){};
  
public:
  void SetCurrent(double val)  {SendCommand(COMM_SET_CUR, val);}
  void SetRpm(double val)	   {SendCommand(COMM_SET_RPM, val);}
  void SetDuty(double val)     {SendCommand(COMM_SET_DUTY, val);}
  void SetBrake(double val)    {SendCommand(COMM_SET_BRAKE, val);}
  void Release()               {SetCurrent(0);}
  void FullBrake()             {SetDuty(0);}
...
private:
  ...
  {
    for(int32_t i=0;i<num;i++) controlSerial.write(arr[i]);
  }
  ...
  void SendCommand(uint8_t command, double val)
  {
    ...
    if(command == COMM_SET_RPM) buffer_append_int32(buff, (int32_t)val, &ind);
    else
    {
      double scale;
